...
- Trạng thái runtime hiện tại đã bổ sung thêm nhóm bảng cho workflow schedule và customer lists qua migration mới:
  ...
  - `customer_lists`
  - `customers`
- Bảng `brands`: migration **`0007`** thêm cột nullable `contact_email`, `phone`, `address` (liên hệ / địa điểm cho AI và prompt ảnh). Code backend đọc các cột này; **nếu DB chưa `alembic upgrade head`**, truy vấn brand có thể lỗi 500 và web báo **Failed to fetch**.
...
| Chiến dịch & nội dung | `campaigns`, `content_items`, `approval_history` | Vòng đời campaign: tạo -> AI viết -> người dùng duyệt |
| Workflow tự động | `workflow_schedules`, `workflow_jobs` | Chạy chiến dịch theo lịch định kỳ |
| Danh sách khách hàng | `customer_lists`, `customers` | Import CSV khách hàng và tái sử dụng |
...
| contact_email | VARCHAR(255) | NULL *(migration `0007`)* |
| phone | VARCHAR(64) | NULL *(migration `0007`)* |
| address | TEXT | NULL *(migration `0007`)* |
| created_at | TIMESTAMPTZ | NOT NULL, DEFAULT `NOW()` |
| updated_at | TIMESTAMPTZ | NOT NULL, DEFAULT `NOW()` |
...
- Sau khi pull code có migration **`0007`**, chạy: `docker compose exec api alembic upgrade head` (hoặc tương đương). Thiếu cột → API lỗi → form brand-vault / `/brands` có thể hiện **Failed to fetch**.
...
- **Luôn chạy migration sau khi cập nhật code có file mới trong `api/alembic/versions/`**. Ví dụ lỗi trên `/brand-vault` hoặc `GET /brands` dạng network **Failed to fetch** thường là do API trả 500 (cột ORM mới chưa tồn tại trong PostgreSQL). Khắc phục: `alembic upgrade head` trên đúng database mà API đang kết nối.
