...
- Trạng thái runtime hiện tại đã bổ sung thêm nhóm bảng cho:
  - Workflow schedule: `workflow_schedules` (có `timezone_name` để hỗ trợ timezone)
  ...
  - **Campaign Ideas**: `campaign_ideas` với các content fields (email_content, post_content, video_script, image_prompt)
...
| Hồ sơ thương hiệu | `brands`, `brand_assets` | Lưu thông tin brand (tên, mô tả, danh mục kinh doanh, khách hàng mục tiêu) để AI hiểu ngữ cảnh chiến dịch |
...
| AI Campaign Assistant | `campaign_ideas` | Lưu ý tưởng chiến dịch + timing + segment được AI gợi ý, và plan chi tiết. Khi hoàn tất, hệ thống sẽ tạo Campaign (bảng `campaigns`) kèm ContentItems cho từng kênh. |
...
erDiagram
    ...
    brands {
        ...
        varchar tone_of_voice          -- legacy: giọng văn (không còn bắt buộc trong UI mới)
        varchar logo_url
        varchar primary_color
        text target_audience
        text[] key_products            -- danh mục kinh doanh: multi-select từ dropdown hoặc tự nhập
        text[] forbidden_words         -- legacy: từ cấm (không còn trong UI mới)
    ...
    campaigns ||--o{ campaign_ideas : related_to
...
| timezone_name | VARCHAR(50) | DEFAULT `'Asia/Ho_Chi_Minh'` |
| is_active | BOOLEAN | NOT NULL, DEFAULT `TRUE` |
...
- `campaign_ideas`: `idx_campaign_ideas_user_id`, `idx_campaign_ideas_status`, `idx_campaign_ideas_brand_id`
...
## 11) API ENDPOINTS MOI - Campaign Ideas (AI Assistant)

| Method | Endpoint | Mo ta |
|--------|----------|--------|
| POST | `/campaign-ideas/generate-brief` | AI tao brief chiến dich tu suggestion |
| GET | `/campaign-ideas` | Danh sach campaign ideas |
| POST | `/campaign-ideas` | Tao campaign idea moi |
| GET | `/campaign-ideas/{id}` | Lay chi tiet campaign idea |
| PATCH | `/campaign-ideas/{id}` | Cap nhat campaign idea |
| DELETE | `/campaign-ideas/{id}` | Xoa campaign idea |
| POST | `/campaign-ideas/{id}/build/email` | AI generate noi dung email |
| POST | `/campaign-ideas/{id}/build/post` | AI generate noi dung Facebook post |
| POST | `/campaign-ideas/{id}/build/video` | AI generate kich ban video |
| POST | `/campaign-ideas/{id}/build/image-prompt` | AI generate prompt tao anh |

## 12) SQL MIGRATION - Runtime Updates

```sql
-- =====================================================
-- RUNTIME MIGRATION - Thêm các columns còn thiếu
-- =====================================================

-- 1. Campaign Ideas - các content fields
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS timing VARCHAR(255);
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS customer_segment TEXT;
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS email_content JSONB;
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS post_content JSONB;
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS video_script JSONB;
ALTER TABLE campaign_ideas ADD COLUMN IF NOT EXISTS image_prompt TEXT;

-- 2. Workflow Schedules - timezone support
ALTER TABLE workflow_schedules ADD COLUMN IF NOT EXISTS timezone_name VARCHAR(50) DEFAULT 'Asia/Ho_Chi_Minh';

-- 3. Campaigns - Content Items API support
-- Endpoint: POST /campaigns/{id}/content-items
-- Bang content_items da co san trong schema

-- =====================================================
-- INDEXES MOI CAN THEM
-- =====================================================
CREATE INDEX IF NOT EXISTS idx_campaign_ideas_user_id ON campaign_ideas(user_id);
CREATE INDEX IF NOT EXISTS idx_campaign_ideas_status ON campaign_ideas(status);
CREATE INDEX IF NOT EXISTS idx_campaign_ideas_brand_id ON campaign_ideas(brand_id);
```

## 13) AI ASSISTANT FLOW - Chi tiết

### Flow 1: AI Campaign Assistant (Wizard)

```
1. Chon Brand
   ↓
2. AI goi y chiến dich (suggestions)
   ↓
3. User chon suggestion + nhap preferences (target customer, duration)
   ↓
4. AI generate brief (title, objective, channels, hook)
   ↓
5. User chinh sua brief (tên, kênh, ưu đãi, mục tiêu)
   ↓
6. Bấm "Bắt đầu tạo nội dung"
   ↓
7. Tao CampaignIdea + Campaign (bang campaigns)
   ↓
8. AI generate noi dung theo tung kênh (Email → Post → Video → Image)
   ↓
9. Luu content vao campaign_ideas + content_items
   ↓
10. Redirect sang trang chi tiet campaign /campaigns/{id}
```

### Data Flow

```
campaign_ideas (luu plan goc)
    ├── email_content (JSON)
    ├── post_content (JSON)
    ├── video_script (JSON)
    └── image_prompt (TEXT)

campaigns (chiến dich chính)
    └── content_items (noi dung đã duyệt)
        ├── channel: 'email'
...
## 14) CONTENT ITEM CHANNEL VALUES

| Channel | Mo ta | Content JSON Structure |
|---------|-------|----------------------|
| `email` | Email marketing | `{ subject, preheader, body, cta_text, cta_url }` |
| `facebook_post` | Bài đăng Facebook | `{ hook, body, hashtags, image_style }` |
| `video_script` | Kịch bản video TikTok | `{ duration, hook_seconds, scenes }` |
| `image_prompt` | Prompt tạo ảnh AI | `{ prompt: "..." }` |

---

> **Huong dan su dung:**
> 1. Ket noi PostgreSQL: `psql -h localhost -U postgres -d aimap`
> 2. Copy SQL o muc 9, 10, 12 va paste vao pgAdmin/DBeaver
> 3. Hoac chay truc tiep: `\i migration.sql`
